...
$variables_metodo = variables_metodo("funcion,id_imagen,fecha_creacion,nombre_imagen,id_categoria,imagen,img_principal,visitas_imagen,palabra_fil,categoria_fil");
...
if ($funcion == "guardar"){
	...
	/*Funcion para guardar los datos del formulario*/	
		guardar($fecha_creacion,$nombre_imagen,$id_categoria,$imagen,$img_principal,$visitas_imagen);
	}elseif($id_imagen!=""){
	/*Funcion para modificar los datos del formulario*/	
		modificar($id_imagen,$fecha_creacion,$nombre_imagen,$id_categoria,$imagen,$img_principal);
	}	
...
function borrar($id)
{
	...
	$result=$db->Execute("SELECT img_previa,img_original,img_full,nombre_imagen FROM ic_galeria_productos WHERE id_imagen = '".$id."' ");
	...
	registrar_cambios($db, date('Y-m-d'), $_SESSION['sess_nombre'], '202', $id, $nombre_imagen, 'ELIMINAR');
	
	$result=$db->Execute("DELETE FROM ic_galeria_productos WHERE id_imagen = '".$id."' ");
	
	if ($result != false) $mensaje = "3";
	else $mensaje  = "0";
		
	echo"<META HTTP-EQUIV='Refresh' CONTENT='0;URL=admin.php?mod=202&mensaje=".$mensaje."'>";
	die();
}
/****************************************************************************************************************************/
function guardar($fecha_creacion,$nombre_imagen,$id_categoria,$imagen,$img_principal)
{
	$path = "adodb/adodb.inc.php";
	include "var.php";
	include "conexion.php";
	...
	if($url_full!="" && $imagen_full!=""){
		...
		if(subir_imagen($alt.$imagen_full, $url_full, $dir))
		{
			...
			echo"<META HTTP-EQUIV='Refresh' CONTENT='1;URL=admin.php?mod=202&mensaje=0'>";
	...
	if($img_principal=="S"){
		$result=update_bd_format(array("img_principal"), "ic_galeria_productos", array("N"), "  WHERE id_categoria='".$id_categoria."'", $db);
	}
	...
	$result=insert_bd_format("fecha_creacion,nombre_imagen,id_categoria,img_previa,img_full,img_original,img_principal", 
							 "ic_galeria_productos", 
							 array($fecha_creacion,$nombre_imagen,$id_categoria1,$img_previa,$img_full,$img_original,$img_principal), 
							 $db);
	...
	//funcion para registrar la modificacion hecha por el usuario
	registrar_cambios($db, date('Y-m-d'), $_SESSION['sess_nombre'], '202', '0', $nombre_imagen, 'GUARDAR');
...
function modificar($id_imagen,$fecha_creacion,$nombre_imagen,$id_categoria,$imagen,$img_principal)
{
	$path = "adodb/adodb.inc.php";
	include "var.php";
	include "conexion.php";
	...
	if($url_full!="" && $imagen_full!=""){
		...
		if(subir_imagen($alt.$imagen_full, $url_full, $dir))
		{
			...
			//Consulto las imagenes creadas para eliminar los archivos fisicos
			$result_del=$db->Execute("SELECT img_previa,img_original,img_full FROM ic_galeria_productos WHERE id_imagen = '".$id_imagen."' ");
	...
	$result=update_bd_format(array("nombre_imagen","fecha_creacion","img_principal","id_categoria",$img_full,$img_previa,$img_original), 
						     "ic_galeria_productos", 
							 array($nombre_imagen,$fecha_creacion,$img_principal,$id_categoria1,$img_full2,$img_previa2,$img_original2), 
	...
	//funcion para registrar la modificacion hecha por el usuario
	registrar_cambios($db, date('Y-m-d'), $_SESSION['sess_nombre'], '202', $id_imagen, $nombre_imagen, 'MODIFICAR');	
	
	implantar_urls($db);
	
	echo"<META HTTP-EQUIV='Refresh' CONTENT='0;URL=admin.php?mod=202&mensaje=".$mensaje."&id_imagen=".$id_imagen."'>";
	die();
}
   ...
   <tr>
    ...
    <td>
	  ...
	  <select name="id_imagen">
		<?php cargar_lista("ic_galeria_productos","id_imagen,nombre_imagen","nombre_imagen","1","","",$db); ?>
      </select>
	  </strong></td>
    <td><strong>
...
if ($id_imagen != ""){
	...
	$sql="SELECT id_imagen,nombre_imagen,fecha_creacion,id_categoria,img_previa,img_full,img_original,img_principal FROM ic_galeria_productos 
	      WHERE id_imagen='".$id_imagen."' ";
	$result=$db->Execute($sql);
	
	list($id_imagen,$nombre_imagen,$fecha_creacion,$id_categoria,$img_previa,$img_full,$img_original,$img_principal)=select_format($result->fields);
...
?>
    <tr>
      <td valign="top"><a href="admin.php?mod=202">Back to list</a></td>
      <td><div align="right"><a href="admin.php?mod=202&funcion=nuevo">Cancel/New</a></div></td>
    </tr>
    <tr>
      <td valign="top">&nbsp;</td>
    ...
    <tr>
      <td valign="top"><div align="right"><strong>Products</strong>(*)</div></td>
      <td>
      <select name="id_categoria[]" size="8" multiple="multiple" id="id_categoria" ultiple="multiple" >
        <?php cargar_lista_multiple("ic_catalogo","id_item,nombre_item","nombre_item","1",$id_categoria,"",$db); ?>
      </select>
      </td>
    </tr>
    <tr>
...
<table width="100%" border="0" cellspacing="3" cellpadding="3">
  <tr>
    <th colspan="7" align="right" bgcolor="#FFFFFF"><a href="admin.php?mod=202&amp;funcion=nuevo">[ New Image]</a></th>
  </tr>
  <tr>
...
?>
  <tr>
    ...
    <td align="center" nowrap="nowrap" bgcolor="#f4f4f4"><a href="admin.php?mod=202&id_imagen=<?=$id?>">[ Edit ]</a></td>
  </tr>
<?php
	$result->MoveNext();
}
