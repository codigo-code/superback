...
@Repository("usuarioSolrLdapDAO")
public class UsuarioSolrLdapDAOImpl implements IUsuarioLdapSolrDAO {

	
	@Autowired
	private SolrServer solrServerUSUARIOSLDAP;
	...
	public boolean addToIndex(UsuarioSolr usuario) {
		...
		try {
			solrServerUSUARIOSLDAP.addBean(usuario);
			solrServerUSUARIOSLDAP.commit();
	...
	public void limpiarIndice(){
		try{
			solrServerUSUARIOSLDAP.deleteByQuery("*:*");
			solrServerUSUARIOSLDAP.commit();			
		}catch(Exception e){
	...
	public boolean addToIndex(Collection<UsuarioSolr> usuarios) {
		boolean success = true;
		try {
			solrServerUSUARIOSLDAP.addBeans(usuarios);
			solrServerUSUARIOSLDAP.commit();
		} catch (MalformedURLException e) {
			logger.error("Error parsing URL address:" + e.getMessage(), e);
			success = false;
	...
	public UsuarioSolr searchByUsername(String username) throws SecurityAccesoDatosException {
		try {
			...
			response = solrServerUSUARIOSLDAP.query(solrQuery);
	...
	public HashMap<String, Usuario> obtenerTodos () throws SecurityAccesoDatosException {
		try {			
			...
			SolrDocumentList documents = obtenerDocumentosSolrPorQuery(Constantes.OBTENER_TODOS_SOLR_LDAP);
			for (SolrDocument document : documents) {
	...
	public void eliminarUsuario(String userName) throws SecurityAccesoDatosException{ 
		try {
			solrServerUSUARIOSLDAP.deleteById(userName);
			solrServerUSUARIOSLDAP.commit();
